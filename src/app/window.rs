...
pub fn setup(mgr: Arc<RwLock<Manager>>) -> MainWindow {
    ...
    let titles: Vec<SharedString> = get_titles().iter().map(|s| SharedString::from(s)).collect();
    window
        .global::<MainWindowAdapter>()
        .set_titles(ModelRc::new(VecModel::from(titles)));
...
fn get_titles() -> Vec<String> {
    ...
    PORTS.iter().for_each(|port| titles.push(port.name()));
    titles
}
